...
select * from perfils;
select * from status_tickets;
select * from ramo_atividades;
select * from empresas;
select * from servicos;
select * from relacionamento_emp_svcs;
select * from tickets;
...
insert into ticket(numero_ticket, data_hora_emissao, codigo_acesso, createdAt, updatedAt, statusTicketId, empresaId, servicoId, prioridade) values(314, sysdate(), CONCAT(DATE_FORMAT(SYSDATE(), '%Y%m%d'),'tw314'), sysdate(), sysdate(), 1, 1, 1, 'Normal');
