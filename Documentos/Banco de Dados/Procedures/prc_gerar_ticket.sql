...
CREATE PROCEDURE `PRC_GERAR_TICKET` (IN  P_EMPRESA_ID 	   INT	  ,
									 IN  P_SERVICO_ID 	   INT	  ,
                                     IN  P_PRIODITARIO BOOLEAN	  ,
                                     OUT P_TICKET	   VARCHAR(10),
                                     OUT P_CODIGO_ACESSO   VARCHAR(10))
BEGIN
		...
		IF (P_EMPRESA_ID IS NOT NULL && P_SERVICO_ID IS NOT NULL && P_PRIORITARIO IS NOT NULL) THEN
			SELECT MAX(NUMERO_TICKET)
			 ...
			 WHERE EMPRESAID = P_EMPRESA_ID
			   ...
			   AND YEAR(DATA_HORA_EMISSAO) = YEAR(SYSDATE())
			   AND MONTH(DATA_HORA_EMISSAO) = MONTH(SYSDATE())
			   AND DAY(DATA_HORA_EMISSAO) = DAY(SYSDATE());
            ...
            -- IF (EXCEPTION = 0) THEN
				...
				IF (EXCEPTION = 0) THEN
					IF (V_SIGLA_SERVICO IS NOT NULL) THEN
						INSERT INTO TICKETS
						VALUES (V_NR_TICKET, SYSDATE(), CONCAT(V_SIGLA_SERVICO,V_NR_TICKET), 
								SYSDATE(), SYSDATE(), 1, P_EMPRESA_ID, P_SERVICO_ID);
						COMMIT;
					END IF;

					SELECT CODIGO_ACESSO, CODIGO_ACESSO
